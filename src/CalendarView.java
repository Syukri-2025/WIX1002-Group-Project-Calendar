...
public class CalendarView {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (dayEvents.isEmpty()) {
                ...
                for (int j = 0; j < dayEvents.size(); j++) {
                    ...
                    if (j > 0) weekView.append("           "); // Indent additional events
                    weekView.append(evt.getTitle())
                           .append(" (")
                           .append(evt.getStart().format(timeFormat))
                           .append(")");
                    if (j < dayEvents.size() - 1) weekView.append("\n");
                }
            }
            weekView.append("\n");
        }
        ...
        if (title != null && dateStr != null && timeStr != null) {
            ...
            if (conflict != null) {
                int proceed = JOptionPane.showConfirmDialog(null,
                    ...
                    "Time: " + conflict.getStart().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")) + 
    ...
    private void addRecurrenceToEvent(Event event) {
        try {
            ...
            if (frequencyStr == null) return; // User cancelled
  ...
  private void showUpdateEventForm() {
    ...
    private void showDeleteEventForm() {
        ...
        // Count how many events have this title
        int count = 0;
        ...
        // If multiple events exist with the same title, ask what to delete
        if (count > 1) {
            String[] options = {"Delete ONE occurrence", "Delete ALL " + count + " occurrences", "Cancel"};
            int choice = JOptionPane.showOptionDialog(null,
                "Found " + count + " events with title '" + title + "'.\nWhat would you like to do?",
            ...
            if (choice == 0) {
                ...
                JOptionPane.showMessageDialog(null, 
                    "Deleted 1 occurrence of '" + title + "'.\n" + (count - 1) + " occurrences remain.",
                    "Delete Successful",
                    JOptionPane.INFORMATION_MESSAGE);
            } else if (choice == 1) {
                // Delete all
...
public void showCalendar(int year, int month) { 
    ...
    private void showSearchMenu() {
        String[] options = {"Search by Date", "Search by Date Range", "Search by Keyword", "Search by Additional Fields"};
